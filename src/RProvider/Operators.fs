...
module Operators =
    ...
    let op_Dynamic (expr:RExpr) (mem:string) =
        try
            ...
            | ActivePatterns.DataFrame Singletons.engine.Value df ->
                SymbolicExpression.column mem df
            | expr -> expr.Member mem
            |> RExprWrapper.toRProvider
        with 
        | :? System.ArgumentOutOfRangeException -> RExprWrapper.toRProvider { ptr = Singletons.engine.Value.Api.nilValue }
