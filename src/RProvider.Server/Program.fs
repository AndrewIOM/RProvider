    ...
    let startServer (pipeName: string) tempFile =
        ...
        let pipeServer =
            new PipeServer<IRInteropServer>(
                Settings.NewtonsoftJsonPipeSerializer(),
                rawPipeStream,
                fun () -> RInteropServer() :> IRInteropServer
            )

        pipeServer.SetLogger(fun a -> LogFile.logf "[Server Pipe log]: %O" a)
        ...
        LogFile.logf "Waiting for parent process pid=%d (%A)" (int parentPid) parentProcess
